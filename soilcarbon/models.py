...
from soilcarbon.helpers.model_helpers import file_is_valid
...
class SourceFile(models.Model):
    ...
    csv_file = models.FileField(upload_to="field_sources/%Y/%m/%d/")
    ...
    def save(self, *args, **kwargs):
        ...
        # ToDo: Check that this is a valid CSV file format before saving this file
        """
        if file_is_valid(self.csv_file):
            if not self.title:
                self.title = slugify(self.title)
                super().save(*args, **kwargs)
            # Todo: add farms or create farm objects that have not been added from previous file uploads
            # background task
